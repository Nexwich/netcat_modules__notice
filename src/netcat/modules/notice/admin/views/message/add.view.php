        ...
        <div class="form">
          <form enctype='multipart/form-data' method='post' action='<?= $MODULE_PATH ?>admin/?controller=message&action=save'>
            ...
            <div class="nc-form-row">
              ...
              <select id="f_Email_From" name='f_Email_From' class="js-selectize" data-select='{"persist":false, "create": true}' data-options='{"load":{"type":"user", "value":<? if ($message['email_from']) { ?>["<?= $message['email_from'] ?>"]<? }else { ?>""<? } ?>}, "essence":"user"}' required='required' <? if ($nc_core->get_settings('SpamUseTransport', 'system') == 'Smtp') { ?> disabled="disabled"<? } ?>></select>
            </div>
            <div class="nc-form-row">
              <label for="f_Email_Reply"><?= OCTOCORP_MODULE_NOTICE_MESSAGE_EMAIL_REPLY ?></label>
              <select id="f_Email_Reply" name='f_Email_Reply' class="js-selectize" data-select='{"persist":false, "create": true}' data-options='{"load":{"type":"user", "value":<? if ($message['email_reply']) { ?>["<?= $message['email_reply'] ?>"]<? }else { ?>""<? } ?>}, "essence":"user"}' required='required' <? if ($nc_core->get_settings('SpamUseTransport', 'system') == 'Smtp') { ?> disabled="disabled"<? } ?>></select>
            </div>
            <div class="nc-form-row">
